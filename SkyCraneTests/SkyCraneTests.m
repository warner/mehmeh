...
- (void) testRoundTripWithUserInfoAndData
{
  [GombotDB clearKeychain];
  [GombotDB updateCredentialsWithAccount:@"andré@example.org" andPassword:@"pässwörd"];
  ...
  NSData* plainOut = [GombotDB decryptData:ciphertext withHMACKey:[GombotDB getKeyForPath:_HMACPATH] andAESKey:[GombotDB getKeyForPath:_AESPATH]];
  NSString* readable = [[NSString alloc] initWithData:plainOut encoding:NSUTF8StringEncoding];
  //NSLog(@"%@", readable);
  
  if (![readable isEqualToString:fileContents]) STFail(@"output did not match input");
}
